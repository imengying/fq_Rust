...
fn build_with_cmake() {
    ...
    // Unicorn's install target is not useful for the Rust bindings, so build the
    // actual library target directly from the cmake build directory.
    let mut config = config;
    let build_target = if cfg!(feature = "dynamic_linkage") {
        "unicorn"
    } else {
        "unicorn_static"
    };
