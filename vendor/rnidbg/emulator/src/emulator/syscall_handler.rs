...
pub const CUSTOM_SVC_SYSCALL_NUMBER: u64 = 0x8877 - 1;

static CUSTOM_SVC_LOG_COUNT: AtomicUsize = AtomicUsize::new(0);

fn debug_custom_svc() -> bool {
    std::env::var("FQ_DEBUG_SIGNER_SVCS").ok().as_deref() == Some("1")
}
...
fn arm64_syscall_handler_unicorn<T: Clone>(
    ...
    let x16 = unicorn
        ...
        .unwrap_or(0);
    if x16 == CUSTOM_SVC_SYSCALL_NUMBER {
        let number = unicorn
            .reg_read(unicorn_engine::RegisterARM64::X12)
            .unwrap();
        unicorn
            .reg_write(unicorn_engine::RegisterARM64::X16, 0)
            .unwrap();
        unicorn
            .reg_write(unicorn_engine::RegisterARM64::X12, 0)
            .unwrap();
        let svc = svc_memory.get_svc(number as u32);
        if let Some(svc) = svc {
            if debug_custom_svc() && CUSTOM_SVC_LOG_COUNT.fetch_add(1, Ordering::Relaxed) < 64 {
                let pc = unicorn
                    .reg_read(unicorn_engine::RegisterARM64::PC)
                    .unwrap_or(0);
                let lr = unicorn
                    .reg_read(unicorn_engine::RegisterARM64::LR)
                    .unwrap_or(0);
                let caller = emulator
                    .find_caller()
                    .map(|module_cell| {
                ...
                eprintln!(
                    "custom svc number={} name={} pc=0x{:X} caller={}",
                    number,
                    svc.name(),
                    pc,
                    caller
    ...
    } else if nr == Some(Syscalls::__NR_io_setup) && swi == 0 && x16 == POST_CALLBACK_SYSCALL_NUMBER
    {
        let number = unicorn
            .reg_read(unicorn_engine::RegisterARM64::X12)
            .unwrap();
        ...
        return;
    } else if nr == Some(Syscalls::__NR_io_setup) && swi == 0 && x16 == PRE_CALLBACK_SYSCALL_NUMBER
    {
        let number = unicorn
            .reg_read(unicorn_engine::RegisterARM64::X12)
            .unwrap();
    ...
    if option_env!("PRINT_SYSCALL_TIME_COST") == Some("1") {
        ...
        if let Some(nr) = nr {
            ...
            unicorn
                .reg_write_i64(unicorn_engine::RegisterARM64::X0, -38)
                .unwrap();
        }
    } else {
        if let Some(nr) = nr {
...
fn arm64_syscall_handler_dynarmic<T: Clone>(swi: i32, emulator: &AndroidEmulator<T>) {
    ...
    let nr = get_syscall(nr_raw);
    let svc_memory = &mut emulator.inner_mut().svc_memory;
    let x16 = backend.reg_read(RegisterARM64::X16).unwrap_or(0);
    if x16 == CUSTOM_SVC_SYSCALL_NUMBER {
        let number = backend.reg_read(RegisterARM64::X12).unwrap();
        backend.reg_write(RegisterARM64::X16, 0).unwrap();
        backend.reg_write(RegisterARM64::X12, 0).unwrap();
        let svc = svc_memory.get_svc(number as u32);
        if let Some(svc) = svc {
            if debug_custom_svc() && CUSTOM_SVC_LOG_COUNT.fetch_add(1, Ordering::Relaxed) < 64 {
            ...
            .add_intr_hook(move |backend, intno| {
                if intno == 1 || intno == 2 || intno == 7 {
                    let x16 = backend
                        .reg_read(unicorn_engine::RegisterARM64::X16)
                        .unwrap_or(0);
                    let swi = if x16 == CUSTOM_SVC_SYSCALL_NUMBER
                        || x16 == POST_CALLBACK_SYSCALL_NUMBER
                        || x16 == PRE_CALLBACK_SYSCALL_NUMBER
                    {
                        0
                    } else {
