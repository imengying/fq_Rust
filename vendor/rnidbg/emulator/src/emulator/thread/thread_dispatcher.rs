...
impl<'a, T: Clone> UniThreadDispatcher<'a, T> {
    ...
    fn run(
        ...
        loop {
            ...
            let previous_tasks = std::mem::take(self.task_list_mut());
            let mut rebuilt_tasks = VecDeque::with_capacity(previous_tasks.len());
            for task_cell in previous_tasks {
                if is_main_finish {
                    rebuilt_tasks.push_back(task_cell);
                    continue;
                }
                let keep_task = match unsafe { &mut *task_cell.get() } {
            ...
            *self.task_list_mut() = rebuilt_tasks;
...
impl<'a, T: Clone> ThreadDispatcher<'a, T> for UniThreadDispatcher<'a, T> {
    ...
    fn run_main_for_result(
        ...
        for task in previous_tasks {
            let keep_task = match unsafe { &*task.get() } {
                AbstractTask::Function64(task_ref) => {
                    if task_ref.is_finish() {
                        task_ref.destroy(&emulator);
                        for signal_task_cell in task_ref.get_signal_task_list() {
                            let signal_task = match unsafe { &mut *signal_task_cell.get() } {
                                AbstractTask::SignalTask(task) => task,
                ...
                AbstractTask::MarshmallowThread(task_ref) => {
                    if task_ref.is_finish() {
                        task_ref.destroy(&emulator);
                        for signal_task_cell in task_ref.get_signal_task_list() {
                            let signal_task = match unsafe { &mut *signal_task_cell.get() } {
