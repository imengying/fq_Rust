...
fn class_from_handle<T: Clone>(dvm: &DalvikVM64<T>, handle: i64) -> Option<Rc<DvmClass>> {
    match jni::get_flag_id(handle) {
        JNI_FLAG_CLASS => dvm.find_class_by_id(&handle).map(|(_, class)| class),
        JNI_FLAG_REF => dvm
            .get_global_ref(handle)
            .map(|object| object.get_class(dvm)),
        JNI_FLAG_OBJECT => dvm
            .get_local_ref(handle)
            .map(|object| object.get_class(dvm)),
        _ => None,
    }
}

fn fallback_super_class<T: Clone>(
    dvm: &DalvikVM64<T>,
    class: &Rc<DvmClass>,
) -> Option<Rc<DvmClass>> {
    ...
    dvm.find_class_by_name("java/lang/Object")
        .map(|(_, object_class)| object_class)
}

fn GetSuperclass<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    let dvm = dalvik!(emulator);
    let class_id = emulator.backend.reg_read(RegisterARM64::X1).unwrap() as i64;
    let class = class_from_handle(dvm, class_id);
    let super_class_id = class
        .as_ref()
        .and_then(|class| {
            class
                .super_class
                .clone()
                .or_else(|| fallback_super_class(dvm, class))
        })
        .map(|class| class.id)
        .unwrap_or(JNI_NULL);
    ...
    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
        debug!(
            "{} {}(class = 0x{:X}) => 0x{:X}",
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("GetSuperclass"),
            class_id,
            super_class_id
        );
    }
...
fn IsAssignableFrom<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    let dvm = dalvik!(emulator);
    let target_id = emulator.backend.reg_read(RegisterARM64::X1).unwrap() as i64;
    let source_id = emulator.backend.reg_read(RegisterARM64::X2).unwrap() as i64;
    let Some(target) = class_from_handle(dvm, target_id) else {
        return RET(JNI_FALSE);
    };
    let Some(source) = class_from_handle(dvm, source_id) else {
        return RET(JNI_FALSE);
    };

    let mut assignable = target.id == source.id || target.name == source.name;
    let mut cursor = source
        .super_class
        .clone()
        .or_else(|| fallback_super_class(dvm, &source));
    while !assignable {
        let Some(class) = cursor else {
            break;
        };
        if class.id == target.id || class.name == target.name {
            assignable = true;
            break;
        }
        cursor = class
    ...
    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
        debug!(
            "{} {}(target = 0x{:X}, source = 0x{:X}) => {}",
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("IsAssignableFrom"),
            target_id,
            source_id,
            if assignable { JNI_TRUE } else { JNI_FALSE }
        );
    }

    RET(if assignable { JNI_TRUE } else { JNI_FALSE })
}
...
fn GetObjectRefType<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    const JNI_INVALID_REF_TYPE: i64 = 0;
    const JNI_LOCAL_REF_TYPE: i64 = 1;
    const JNI_GLOBAL_REF_TYPE: i64 = 2;

    let dvm = dalvik!(emulator);
    let object_id = emulator.backend.reg_read(RegisterARM64::X1).unwrap() as i64;
    let ret = match jni::get_flag_id(object_id) {
        JNI_FLAG_CLASS => JNI_LOCAL_REF_TYPE,
        JNI_FLAG_OBJECT => {
            if dvm.get_local_ref(object_id).is_some() {
                JNI_LOCAL_REF_TYPE
            } else {
                JNI_INVALID_REF_TYPE
            }
        }
        JNI_FLAG_REF => {
            if dvm.get_global_ref(object_id).is_some() {
                JNI_GLOBAL_REF_TYPE
            } else {
                JNI_INVALID_REF_TYPE
            }
        }
        _ => JNI_INVALID_REF_TYPE,
    };
    ...
    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
        debug!(
            "{} {}(object = 0x{:X}) => {}",
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("GetObjectRefType"),
            object_id,
            ret
        );
    }
...
pub fn initialize_env<T: Clone>(svc_memory: &mut SvcMemory<T>) -> u64 {
    ...
    let _get_superclass =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetSuperclass", GetSuperclass));
    let _is_assignable_from =
        svc_memory.register_svc(SimpleArm64Svc::new("_IsAssignableFrom", IsAssignableFrom));
    ...
    let _call_object_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallObjectMethod",
        NoImplementedHandler,
    ));
    ...
    let _call_boolean_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallBooleanMethod",
        NoImplementedHandler,
    ));
    ...
    let _call_int_method =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallIntMethod", NoImplementedHandler));
    ...
    let _call_void_method =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallVoidMethod", NoImplementedHandler));
    ...
    let _get_object_ref_type =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetObjectRefType", GetObjectRefType));
